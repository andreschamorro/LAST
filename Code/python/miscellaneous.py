...
def blosumMat(filename, filetype, ignoreline=[], printer=True):
    ...
    align_array_t = numpy.array(
        [list(rec) for rec in alignment if rec.id not in ignoreline],
        numpy.character).T
    numblock = 0
    Cij = numpy.zeros((4, 4))
    for col in align_array_t:
        ...
        if counter[b'-'] == 0:
            numblock += 1
            Cijk[0,
                 :] = counter[b'A']*numpy.array([counter[b'A'],
                                                 counter[b'C'],
                                                 counter[b'G'],
                                                 counter[b'T']])
            Cijk[1,
                 :] = counter[b'C']*numpy.array([counter[b'A'],
                                                 counter[b'C'],
                                                 counter[b'G'],
                                                 counter[b'T']])
            Cijk[2,
                 :] = counter[b'G']*numpy.array([counter[b'A'],
                                                 counter[b'C'],
                                                 counter[b'G'],
                                                 counter[b'T']])
            Cijk[3,
                 :] = counter[b'T']*numpy.array([counter[b'A'],
                                                 counter[b'C'],
                                                 counter[b'G'],
                                                 counter[b'T']])
